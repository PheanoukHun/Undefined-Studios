...
img_list1 = [pygame.transform.scale(pygame.image.load(file).convert_alpha(), (TILE_SIZE, TILE_SIZE)) for file in img_list]
button_list = []
...
save_button = Button(SCREEN_WIDTH // 2, SCREEN_HEIGHT + (LOWER_MARGIN // 4), "LevelEditor/LevelEditor-main/LevelEditor-main/img/save_btn.png")
load_button = Button(SCREEN_WIDTH // 2 + 200, SCREEN_HEIGHT + (LOWER_MARGIN // 4), "LevelEditor/LevelEditor-main/LevelEditor-main/img/load_btn.png")
...
def draw_world():
    for y, row in enumerate(world_data):
        for x, tile in enumerate(row):
            if tile >= 0:
                screen.blit(img_list1[tile], (x * TILE_SIZE - scroll, y * TILE_SIZE))
